...
function DerivedFieldDialog(props) {
	...
	useEffect(() => {
		...
		const ownFieldsArray = loadedDerivedFields.filter(
			item => !item.isPublic && item.userName === user.current.name
		);
		const otherFieldsArray = loadedDerivedFields.filter(
			item => !item.isPublic && item.userName !== user.current.name
		);

		publicFieldsArray.forEach(field =>
			publicFields.current.set(field.name, field)
		);
		ownFieldsArray.forEach(field => ownFields.current.set(field.name, field));
		otherFieldsArray.forEach(field =>
			otherFields.current.set(field.name, field)
		);
	...
	const disabledSaveAllButton = useMemo(() => {
		...
		if (
			...
			listOfChangedFields.current.length > 0
		) {
	...
	function onClose() {
		...
		if (needAsk) {
			props.showAlertDialog(
				`Внимание, есть несохраненные поля. Вы действительно хотите выйти?`,
				null,
				null,
				answer => onCloseAnswer(answer)
			);
		} else {
			onCloseAnswer(true);
		}
	...
	function handleSaveNewField(obj) {
		saveNewField(
			...
			reportId,
			user.current.name,
			listOfChangedFields.current,
			otherDerivedFields,
			(ok, data, originalData, str, variant, changedList) => {
				if (ok) {
	...
	function handleSaveAllFields() {
		saveAllFields(
			...
			listOfChangedFields.current,
			otherDerivedFields,
			(ok, data, str, variant, originalData, changedList) => {
				if (ok) {
					enqueueSnackbar(str, variant);
					props.onCancel(true);
				} else {
	...
	function handleDelete(id) {
		if (id === 'new') {
			setEditedDerivedFields(
				editedDerivedFields.filter(item => item.id !== 'new')
			);
			listOfChangedFields.current = listOfChangedFields.current.filter(
				item => item.id !== 'new'
			);
			setActiveIndex(null);
		} else {
				...
				{loading ? (
					...
					<>
						...
						<DerivedFieldDialogForm
							...
							onSave={obj =>
								obj.id === 'new'
									? handleSaveNewField(obj)
									: handleSaveEditField(obj)
							}
						/>
