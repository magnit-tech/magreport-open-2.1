...
export default function PivotDataProvider(jobId, onTableDataReady, onTableDataLoadFailed) {
    ...
    let filterGroup = new PivotFilterGroup();
    ...
    */
    ...
    this.loadDataForNewFieldsLists = (newFieldsLists, newFilterGroup, newMetricFilterGroup, newValuesToSort, columnFrom, columnCount, rowFrom, rowCount) => {
        fieldsLists = newFieldsLists;
        filterGroup = newFilterGroup;
        metricFilterGroup = newMetricFilterGroup;
        valuesToSort = newValuesToSort
    ...
    function loadDataForRecWindow(loadingType) {
        ...
        let request = buildOlapRequest(fieldsLists, filterGroup, metricFilterGroup, valuesToSort, loadingWindow);
    ...
    this.changeWindow = (columnFrom, columnCount, rowFrom, rowCount) => {
        ...
        if(loadingStatus < 3){
            ...
            if(reqWindow.columnFrom + reqWindow.columnCount <= loadingWindow.columnFrom + loadingWindow.columnCount &&
                ...
                this.loadDataForNewFieldsLists(fieldsLists, filterGroup, metricFilterGroup, valuesToSort, columnFrom, columnCount, rowFrom, rowCount);
            }
            return false;
        }
    ...
    */
    ...
    function buildOlapRequest(fieldsLists, filterGroup, metricFilterGroup, valuesToSort, win){
        ...
        if (filterGroup){
            rqst['filterGroup'] = filterGroup
        };

        if (metricFilterGroup){
            rqst['metricFilterGroup'] = metricFilterGroup
        };

        if (valuesToSort) {
            rqst[valuesToSort.name] = valuesToSort.data
        }

        if (Object.keys(valuesToSort).length !== 0) {
            const {rowSort, columnSort} = valuesToSort

            if (rowSort && rowSort.data[0].order !== '') rqst['rowSort'] = rowSort.data

            if (columnSort && columnSort.data[0].order !== '') rqst['columnSort'] = columnSort.data
        }
        /*
        if (fieldsLists.filterFields.filter(item => item.aggFuncName === "").length > 0) {
            rqst['filterGroup'] = {
                operationType: 'AND',
                invertResult: false,
                childGroups: [],
                filters: fieldsLists.filterFields.filter(item => item.aggFuncName === "").map((v) => ({
                    fieldId : v.fieldId,
        ...
        if (fieldsLists.filterFields.filter(item => item.aggFuncName !== "").length > 0) {
            rqst['metricFilterGroup'] = {
                operationType: 'AND',
                invertResult: false,
                childGroups: [],
                filters: fieldsLists.filterFields.filter((item) => item.aggFuncName !== "").map((v, i) => ({
                    metricId : i,
                    filterType: v.filter.filterType,
                    invertResult: v.filter.invertResult,
                    values:  v.filter.values 
