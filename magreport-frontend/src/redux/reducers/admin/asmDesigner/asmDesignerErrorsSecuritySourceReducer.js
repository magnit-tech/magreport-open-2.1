...
export const asmDesignerErrorsSecuritySourceReducer = (state, action) => {
    ...
    switch (action.type) {
        case ASM_DATA_LOADED:
            ...
            return {
                ...
                [FIELDS]: (data[FIELDS] ? data[FIELDS] : []).map(fieldWrapper => {
                    ...
                    if (FIELD_TYPE === DOMAIN_NAME_FIELD && data[IS_DEFAULT_DOMAIN]){
                        return {
     ...
     /*   case ASM_DESIGNER_CHANGE_ROOT_DATA:
            if (action.key === IS_DEFAULT_DOMAIN){
                if (action.value){
                    return {
                        ...state,
...
*/
        ...
        case ASM_DESIGNER_DELETE_SECURITY_SOURCE_FIELD:
            let newFields = state.fields.filter((fieldItem, index) => index !== action.fieldIndex);
            return {...state,
                [FIELDS]: newFields
            };
