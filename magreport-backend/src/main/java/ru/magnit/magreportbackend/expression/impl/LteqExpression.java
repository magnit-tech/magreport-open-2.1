...
public class LteqExpression extends ParameterizedExpression {
    private final Pair<String, DataTypeEnum> result = new Pair<>(null, DataTypeEnum.BOOLEAN);

    public LteqExpression(FieldExpressionResponse fieldExpression, ExpressionCreationContext context) {
    ...
    public Pair<String, DataTypeEnum> calculate(int rowNumber) {
        ...
        switch (firstValue.getR()) {
            ...
            case STRING, TIMESTAMP, DATE -> {
                final var compareResult = firstValue.getL().compareToIgnoreCase(secondValue.getL());
                if (compareResult <= 0) {
                    result.setL(TRUE);
                } else {
                    result.setL(FALSE);
            ...
            case BOOLEAN -> {
                if (firstValue.getL().equals(FALSE) && secondValue.getL().equals(TRUE) ||
                    firstValue.getL().equals(secondValue.getL())) {
                    result.setL(TRUE);
                } else {
                    result.setL(FALSE);
                }
