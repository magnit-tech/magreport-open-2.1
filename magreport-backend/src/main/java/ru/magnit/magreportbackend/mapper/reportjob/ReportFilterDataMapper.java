...
public class ReportFilterDataMapper implements Mapper<ReportFilterData, FilterReport> {
    ...
    private List<ReportFilterLevelData> getLevels(List<FilterReportField> fields, Long filterTypeId) {
        ...
        return levels
            ...
            .map(level -> new ReportFilterLevelData(
                ...
                fields.stream().filter(field -> field.getFilterInstanceField().getLevel().equals(level)).filter(field -> FilterFieldTypeEnum.getByOrdinal(field.getFilterInstanceField().getTemplateField().getType().getId()) == ID_FIELD).map(this::getDataSetFieldId).findFirst().orElse(-1L),
                ...
                fields.stream().filter(field -> field.getFilterInstanceField().getLevel().equals(level)).filter(field -> FilterFieldTypeEnum.getByOrdinal(field.getFilterInstanceField().getTemplateField().getType().getId()) == CODE_FIELD).map(this::getDataSetFieldId).findFirst().orElse(-1L),
    ...
    private Long getDataSetFieldId(FilterReportField field) {
        final var datasetField = field.getFilterInstanceField().getDataSetField();
        return datasetField == null ? -1L : datasetField.getId();
    }
}
