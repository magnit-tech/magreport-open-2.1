...
public class InitConfig {
    ...
    public void initSuperUser() {
        ...
        var roles = userDomainService.getUserRoles(username, authConfig.getDefaultDomain(), UserRoleTypeEnum.MANUAL).stream().map(RoleView::getName).collect(Collectors.toList());

        if (roles.stream().noneMatch(r -> r.equals("ADMIN"))) {
            roles.add("ADMIN");
            userDomainService.setUserRoles(username, authConfig.getDefaultDomain(), roles, UserRoleTypeEnum.MANUAL);
        }
    }
