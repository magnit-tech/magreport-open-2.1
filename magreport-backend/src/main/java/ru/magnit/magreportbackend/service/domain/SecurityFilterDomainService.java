...
public class SecurityFilterDomainService {
    ...
    private final SecurityFilterReportFieldsMappingRepository fieldsMappingRepository;
    ...
    @Transactional
    public List<ReportJobFilterData> getEffectiveSettingsForReport(Long reportId, Set<Long> roleSet) {
        final var fieldMappings = fieldsMappingRepository.getFieldsMappings(reportId);
        if (fieldMappings.isEmpty()) return Collections.emptyList();
        final var securityFilterIds = fieldMappings.stream().map(SecurityFilterFieldMapping::securityFilterId).distinct().toList();
        final var securityFilters = repository.getAllByIdIn(securityFilterIds);
        final var fieldNames = new HashMap<Long, String>();
        fieldMappings.forEach(entry -> fieldNames.put(entry.filterInstanceFieldId(), entry.fieldName()));

        return securityFilters
            .stream()
            .flatMap(securityFilter -> securityFilter.getFilterRoles()
                .stream()
                .filter(filterRole -> roleSet.contains(filterRole.getRole().getId())))
            .map(filterRole -> new ReportJobFilterData(
                -1L,
                -1L,
                -1L,
                "",
                ...
                filterRole.getFilterRoleTuples()
                    ...
                    .map(tuple -> new ReportJobTupleData(tuple.getTupleValues()
                        .stream()
                        .map(value -> new ReportJobTupleFieldData(
                            value.getField().getId(),
                            -1L,
                            -1L,
                            value.getField().getLevel(),
                            mapFieldName(value, fieldNames),
                            value.getField().getDataSetField().getType().getEnum(),
                            value.getValue()
                        )).toList())).toList()
            ))
            .toList();
    ...
    @Transactional
    public List<ReportJobFilterData> getEffectiveSettingsForFilter(Long datasetId, Set<Long> roleSet) {
        final var securityFilters = repository.getAllByDataSetIdsAndRoleIds(Collections.singletonList(datasetId), roleSet);
                ...
                filterRole.getFilterRoleTuples()
                    ...
                    .map(tuple -> new ReportJobTupleData(tuple.getTupleValues()
                        ...
                        .map(value -> new ReportJobTupleFieldData(
                            ...
                            -1L,
                            value.getField().getLevel(),
                            value.getField().getDataSetField().getName(),
                            value.getField().getDataSetField().getType().getEnum(),
                            value.getValue()
                        )).toList())).toList()
            ))
    ...
    public List<SecurityFilterResponse> checkSecurityFilterExistsForDataset(Long dataSetId) {
        ...
        var filters = securityFilterDataSets.stream().map(SecurityFilterDataSet::getSecurityFilter).toList();

        return filters.stream().map(securityFilterResponseMapper::from).toList();
    }
    ...
    @Transactional
    public Map<Long, String> getReportFieldsMapping(Long reportId) {
        final var queryResult = fieldsMappingRepository.getFieldsMappings(reportId);

        final var result = new HashMap<Long, String>();
        queryResult.forEach(entry -> result.put(entry.filterInstanceFieldId(), entry.fieldName()));
    ...
    private String mapFieldName(SecurityFilterRoleTupleValue value, HashMap<Long, String> fieldNames) {
        final var fieldId = value.getField().getId();
        if (!fieldNames.containsKey(fieldId) || fieldNames.get(fieldId) == null) {
            final var filterName = value.getTuple().getSecurityFilterRole().getSecurityFilter().getName();
            final var fieldName = value.getField().getName();
            throw new InvalidParametersException("Невозможно получить маппинг поля '" + fieldName + "' для фильтра безопасности '" + filterName + "'");
        }
