...
public class FilterInstanceService {
    ...
    public FilterInstanceChildNodesResponse getChildNodes(ChildNodesRequest request) {
        ...
        final var currentUserRoles = userDomainService.getCurrentUserRoles(null);
        final var dataSetId = domainService.getDataSetId(request.getFilterId());
        final var effectiveSettings = securityFilterDomainService.getEffectiveSettingsForFilter(dataSetId, currentUserRoles.stream().map(RoleView::getId).collect(Collectors.toSet()));
