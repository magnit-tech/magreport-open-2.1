...
public class UserServicesController {
    ...
    public static final String DOMAIN_LIST_SERVICE = "/api/v1/user-services/get-domain-list";
    ...
    @Operation(summary = "Получение списка доменов, для пользователей которых настроена аутентификация")
    @ResponseStatus(HttpStatus.OK)
    @PostMapping(value = DOMAIN_LIST_SERVICE,
        produces = APPLICATION_JSON_VALUE)
    public ResponseList<DomainResponse> getDomainList() {
        LogHelper.logInfoUserMethodStart();

        var response = ResponseList.<DomainResponse>builder()
            .success(true)
            .message("")
            .data(service.getDomains())
            .build();
