...
public interface ReportJobRepository extends JpaRepository<ReportJob, Long> {
    ...
    @Query(value = "SELECT RJ.REPORT_JOB_ID," +
            ...
            "         LEFT JOIN REPOSITORY.OLAP_USER_CHOICE OUC ON OUC.REPORT_ID = RJ.REPORT_ID AND OUC.USER_ID = RJ.USER_ID",
            nativeQuery = true)
    ...
    @Modifying
    @Query(value = "update REPOSITORY.REPORT_JOB set REPORT_JOB_STATUS_ID=:scheduledId where REPORT_JOB.REPORT_JOB_STATUS_ID=:pendingId AND REPORT_JOB.MODIFIED<=:updateBefore", nativeQuery = true)
    void updatePendingJobs(@Param("pendingId") Long pendingId, @Param("scheduledId") Long scheduledId, @Param("updateBefore") LocalDateTime updateBefore);
}
