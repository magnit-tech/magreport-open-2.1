...
class ExternalAuthUserRoleRefreshServiceTest {
    ...
    void refreshUserRoles() {
        ...
        verify(userService, times(2)).loginUser(any(), anyString(), any());
        verify(userService, times(2)).clearRoles(anyString(),anyString(), any());
        verify(userService).setUserRoles(anyString(), anyString(), any(), any());
        verify(queryBuilder).buildQuery(any(), anyBoolean());
